...
func (s *AdminUserService) RefreshToken(ctx context.Context, refreshToken string) (*v1.LoginResponse, error) {
	...
	case enum.UserTypeUser:
		...
		// 获取用户角色
		roles, roleErr := s.userRepo.GetUserRoles(ctx, user.ID)
		if roleErr != nil {
			// 如果获取角色失败，使用默认角色
			roles = []string{"user"}
		}
		...
		// 生成新的访问令牌
		accessToken, err := s.authService.GenerateAccessTokenWithContext(ctx, uint(user.ID), user.Username, enum.UserTypeUser, roles)
		if err != nil {
			return nil, err
		}

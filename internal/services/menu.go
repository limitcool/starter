...
func (s *MenuService) CreateMenu(menu *model.Menu) error {
	return sqldb.Instance().DB().Create(menu).Error
}

// UpdateMenu 更新菜单
func (s *MenuService) UpdateMenu(menu *model.Menu) error {
	return sqldb.Instance().DB().Model(&model.Menu{}).Where("id = ?", menu.ID).Updates(menu).Error
}
...
func (s *MenuService) DeleteMenu(id uint) error {
	// 检查是否有子菜单
	var count int64
	if err := sqldb.Instance().DB().Model(&model.Menu{}).Where("parent_id = ?", id).Count(&count).Error; err != nil {
		return err
	}
	if count > 0 {
	...
	// 删除菜单关联的角色
	if err := sqldb.Instance().DB().Where("menu_id = ?", id).Delete(&model.RoleMenu{}).Error; err != nil {
		return err
	}

	// 删除菜单
	return sqldb.Instance().DB().Delete(&model.Menu{}, id).Error
}
...
func (s *MenuService) GetMenusByRoleID(roleID uint) ([]*model.Menu, error) {
	...
	// 查询菜单
	var menus []*model.Menu
	err = sqldb.Instance().DB().Where("id IN ?", menuIDs).Order("order_num").Find(&menus).Error
	if err != nil {
		return nil, err
	}
...
func (s *MenuService) GetUserMenus(userID uint) ([]*model.Menu, error) {
	...
	// 2. 获取角色关联的菜单ID
	var roleMenus []model.RoleMenu
	err = sqldb.Instance().DB().Where("role_id IN ?", roleIDs).Find(&roleMenus).Error
	if err != nil {
		return nil, err
	}
	...
	// 3. 查询菜单信息
	var menus []*model.Menu
	err = sqldb.Instance().DB().Where("id IN ? AND status = ? AND type IN ?", menuIDs, 1, []int8{0, 1}).Order("order_num").Find(&menus).Error
	if err != nil {
		return nil, err
	}
...
func (s *MenuService) GetMenuPermsByUserID(userID uint) ([]string, error) {
	...
	// 查询菜单权限标识
	var perms []string
	err = sqldb.Instance().DB().Model(&model.Menu{}).
